...
def make_video_from_frames(img_dir, output_path, fps=10):
    ...
    pattern = os.path.join(img_dir, 'frame_*_BEV_heatmap.png')
    files = glob.glob(pattern)
    files.sort(key=lambda p: int(re.search(r'frame_(\d+)_BEV_heatmap', os.path.basename(p)).group(1)))
